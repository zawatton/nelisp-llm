...
Returns (OUT TAPE)."
  (let* ((bc (plist-put bc :layer layer))
         ...
         (rdims (plist-get cfg :rope-dims)) (rbase (plist-get cfg :rope-base))
         (eps (or (plist-get cfg :rms-eps) 1.0e-6))
      ...
      (let* ((s (aref (plist-get tape :gated) tt))
        ...
        (dotimes (i qdim)
          (let* ((gi (aref gate (+ (* tt qdim) i)))
            ...
            (aset dctx (+ (* tt qdim) i) (* (aref dg i) (nl-llm-dn--silu gi)))
            (aset dgate (+ (* tt qdim) i)
                  (* (aref dg i) ci (nl-llm-dn--silu-d gi)))))))
...
(defun nl-llm-bonsai-bw-block-forward (bc layer x seq)
  ...
  (let ((iv (plist-get (plist-get bc :cfg) :full-attention-interval)))
