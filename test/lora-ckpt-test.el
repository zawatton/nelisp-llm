...
;;   emacs -Q --batch -L lisp -L ../nelisp-photon/lisp -l test/lora-ckpt-test.el
...
(defun lck--mk-block (dim kvdim ff seed)
  (list :ln1g (lck--t (list dim) (+ seed 1))
        :wq   (lck--t (list dim dim) (+ seed 2))
        :bq   (lck--t (list dim) (+ seed 3))
        :wk   (lck--t (list kvdim dim) (+ seed 4))
        :bk   (lck--t (list kvdim) (+ seed 5))
        :wv   (lck--t (list kvdim dim) (+ seed 6))
        :bv   (lck--t (list kvdim) (+ seed 7))
        :wo   (lck--t (list dim dim) (+ seed 8))
        :bo   (lck--t (list dim) (+ seed 9))
        :ln2g (lck--t (list dim) (+ seed 10))
        :wg   (lck--t (list ff dim) (+ seed 11))
        :bg   (lck--t (list ff) (+ seed 12))
        :wu   (lck--t (list ff dim) (+ seed 13))
        :bu   (lck--t (list ff) (+ seed 14))
        :wd   (lck--t (list dim ff) (+ seed 15))
        :bd   (lck--t (list dim) (+ seed 16))))
...
(defun lck--size-model ()
  (let ((dim 128)
        ...
        (ff 256)
        (vocab 64))
    (list :config (list :dim dim :heads 8 :kv-heads 4 :ff ff :vocab vocab :nblocks 2)
          :step 23
          :wte (lck--t (list vocab dim) 100)
          :lnfg (lck--t (list dim) 200)
          :bh (lck--t (list vocab) 300)
          :blocks (list (lck--mk-block dim kvdim ff 1000)
                        (lck--mk-block dim kvdim ff 2000)))))

(defun lck--size-adapters ()
  (list
   (cons :block-0-wq (lck--adapter 2 128 128 500))
   (cons :block-0-wv (lck--adapter 2 128 64 600))
   (cons :block-0-wo (lck--adapter 2 128 128 700))
   (cons :block-0-wg (lck--adapter 2 128 256 800))
   (cons :block-0-wu (lck--adapter 2 128 256 900))
   (cons :block-0-wd (lck--adapter 2 256 128 1000))
   (cons :block-1-wq (lck--adapter 2 128 128 1500))
   (cons :block-1-wv (lck--adapter 2 128 64 1600))
   (cons :block-1-wo (lck--adapter 2 128 128 1700))
   (cons :block-1-wg (lck--adapter 2 128 256 1800))
   (cons :block-1-wu (lck--adapter 2 128 256 1900))
   (cons :block-1-wd (lck--adapter 2 256 128 2000))))
  ...
  (unwind-protect
      (progn
        (nl-llm-lora-ckpt-save adapter-path adapters
                               (list :step 23 :base-config (plist-get (lck--size-model) :config)))
        (nl-llm-ckpt-save full-path (lck--size-model))
