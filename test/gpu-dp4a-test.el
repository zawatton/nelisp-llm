...
(let* ((n 16)
  ...
  (let* ((seq 4) (in 17) (out 6) (sc 0.5)
         ...
         (gpu (nl-llm-bitnet-dp4a-linear x w bias))
         (gpu1f (nl-llm-bitnet-dp4a-1f-linear x w bias)) (maxrel 0.0) (mr1f 0.0))
    (nl-llm-gpu-disable)
        ...
        (let ((dot 0))
          (dotimes (i in)
            (let ((a8 (lane alo2 ahi2 (+ (* s ng 4) i))) (w8 (lane wlo whi (+ (* o ng 4) i))))
              (setq dot (+ dot (* a8 w8)))))
          (let ((want (+ (aref bd o) (* beta (aref gamma s) dot))) (idx (+ (* s out) o)))
            (setq maxrel (max maxrel (/ (abs (- want (aref gpu idx))) (max 1e-3 (abs want)))))
            (setq mr1f (max mr1f (/ (abs (- want (aref gpu1f idx))) (max 1e-3 (abs want))))))))))
    (dp--ck "bitlinear-dp4a (2 f32/4 lanes) == CPU int8 ternary" (< maxrel 1e-5) (format "maxrel=%.2e" maxrel))
    (dp--ck "bitlinear-dp4a-1f (bitcast, 1 f32/4 lanes) == CPU" (< mr1f 1e-5) (format "maxrel=%.2e (4x less weight memory)" mr1f)))
